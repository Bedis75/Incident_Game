...
export default function GameBoardClient() {
  ...
  }, [apiBaseUrl, sessionCode, playerId]);
  ...
  useEffect(() => {
    ...
    const hasLastRollFallbackTrigger =
      rollSequence === 0 && lastRoll !== null && previousLastRollRef.current !== lastRoll;
  ...
  const teamsForRender = isDiceRolling && frozenTeams ? frozenTeams : gameState?.teams || [];
  ...
  const busy = isActionLoading || !gameState || isLoading;
          ...
          <article className="board-panel board-main board-main-modern">
            ...
            <div className="ig-track" role="img" aria-label="24-space board with category spaces and tokens">
              ...
              {SPACES.map((space, index) => (
                <div key={index} className={`ig-space ${categoryClass[space]} ig-pos-${index}`}>
